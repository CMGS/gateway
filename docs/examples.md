...
The response has `content:null` and `finish_reason:"tool_calls"` with the call
in `choices[0].message.tool_calls`.
...
curl -s 'localhost:8080/internal/ledger?limit=5' -H 'authorization: Bearer ak-demo-123'
```
